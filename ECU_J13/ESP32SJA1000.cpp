...
int ESP32SJA1000Class::endPacket()
{
  ...
  // wait for TX complete
    while ((readRegister(REG_SR) & 0x08) != 0x08) { // 00001000 Transmission complete status. 0 = incomplete
    ...
    if (readRegister(REG_SR) == 0x30) { // e.g. if no transceiver is connected REG_SR=48 (0b00110000) ECC=0
      modifyRegister(REG_CMR, 0x1f, 0x02); // error, abort
      Serial.println(F("D227 abort CAN DESCONECTADA")); Serial.println(readRegister(REG_SR));
      return 0;
    }
    yield();
    }
